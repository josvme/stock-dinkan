...
object StockSaverManager extends IOApp {
  ...
  def readAndWriteUntilEnd[F[+_]: Monad](io: F[Option[Result]], manager: InMemoryReadWritePort[F]): F[Result] = {
      io.flatMap {
        case Some(msg) => {
          val t: F[Option[Result]] = readWriteStockData[F](manager).map(x => {println(x); x})
  ...
  def readWriteStockData[F[+_]: Monad](manager: InMemoryReadWritePort[F]): F[Option[Result]] = {
